...
test_that("Class construction edge cases function correctly", {
  ...
  expect_equal(
    number_img(
      img, "n", 5,
      rlang::set_attrs(4:6, auto = FALSE), "med"
    ),
    number_img(
      img, "n", 5,
      rlang::set_attrs(4:6, auto = rep(FALSE, 3)), rep("median", 3)
    )
  )
  expect_error(
    number_img(img, "n", 5, rlang::set_attrs(4:6, auto = NA), NA),
    "swaps.*att.*not NA"
  )
  expect_error(
    number_img(
